...
switch (locationKey){
  ...
  case "Могилевская областная детская больница":
    ...
    break;
  case "РНПЦ":
    let tdRnpc = [...alltr].slice(158,170);
    allTdContent(tdRnpc);
    break;
  case "Admin":
    let tdAdmin = [...alltr].slice(1,170);
...
let td1Elements = document.querySelectorAll('td');

for(let td of td1Elements) {
  if(td.innerText=='ГУ "Республиканская клиническая больница медицинской реабилитации"') {
    td.style.display = 'block';
    td.style.width = '200px';
    td.style.fontWeight = 'bold';
    td.style.fontSize = '14px';
