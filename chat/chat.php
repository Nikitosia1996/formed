...
//if(isset($_COOKIE['login']) && ($_COOKIE['login'] == "admin@admin.by" || $_COOKIE['login'] == "lendzin@rnpcmt.by")){
//    define( "DBFILE", realpath( str_replace( '\\', '/', __DIR__ ) ) . "/chatal.db" );
//}
//else {
...
define( "HEADER", "Общий чат" ); //Заголовок
...
define( "MAXUSERTEXTLEN", 2024 ); //Максимальная длина сообщения пользователя
		...
		<div id="wrapper">
			...
			<form action="" method="post" id="sendForm">
				<div id="sendDialog" class="block2">
					<input type="text" name="name" readonly value="<?php echo( $_COOKIE['login'] ); ?>" maxlength="<?php echo( MAXUSERNAMELEN ) ?>" placeholder="Авторизуйтесь" />
			...
			( function() {
				...
				f.onsubmit = function() {
					if ( /^\s*$/.test( name.value ) ) {
						tipUpper( name, "Пожалуйста, авторизуйтесь" );
						return false;
					}
